...
class MTGNN(nn.Module):
    ...
    def __init__(self, in_dim, num_nodes, horizon, batch_size, seq_length,device, gcn_true = True, buildA_true = 'buildA_true', gcn_depth = 2, static_feat=None, dropout=0.3, subgraph_size=20, node_dim=40, dilation_exponential=1, conv_channels=32, residual_channels=32, skip_channels=64, end_channels=128, out_dim=12, layers=3, propalpha=0.05, tanhalpha=3, layer_norm_affline=True):
    ...
    def forward(self, x):
        ...
        idx = None
        x = x.squeeze(2)
        history_data = x.transpose(1, 3).contiguous()
        ...
        # print(x.shape)
        x = self.end_conv_2(x)
        x = x.squeeze (3).transpose (1, 2)[:,:,:self.horizon]
        # print(x.shape)
